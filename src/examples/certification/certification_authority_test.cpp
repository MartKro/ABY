...
int main(int argc, char** argv) {
    ...
    ///////////////////////////////////////////////////////////
	//TEST ONLY; DON'T INIT ON REAL APPLICATION

    crypto* crypt_test = new crypto(AES_BITS, (uint8_t*) const_seed);

	//Init the input of the party who desires the inputs to be certified
	CBitVector input_a_test, key_test, random_counter_test;

	input_a_test.CreateBytes(AES_BYTES * nvals, crypt_test);
	key_test.CreateBytes(AES_BYTES, crypt_test);
    ...
    //Set certified input here
    input_a = input_a_test.GetArr(); //For testing
    //input_a = ... //For real application
	...
	connection->Receive(binding_key, AES_BYTES);

    //Set random values here
    key = key_test.GetArr(); //For testing
    //key = random_method //For real application
    random_counter = random_counter_test.GetArr(); //For testing
    //commit_key = random_method //For real application

    //AES counter mode: 120 bit nounce, 8 bit counter,
    //initial counter is random
    ...
    uint8_t* expanded_binding_key = new uint8_t[AES_EXP_KEY_BYTES];
    ...
    //Use Matyas–Meyer–Oseas (without chaning the key) with AES to commit the key
	verify_AES_encryption(key, expanded_binding_key, 1,
	    committed_ciphertext, crypt);
    for(uint32_t i = 0; i < AES_BYTES; ++i) {
        committed_ciphertext[i] ^= key[i];
    }
    ...
    for(uint32_t i = 0; i < AES_BYTES; ++i) {
        sign_values[(nvals + 2) * AES_BYTES + i] = binding_key[i];
    }
    ...
    //prepare the message = key, random_counter, sig
    uint8_t* msg = new uint8_t[AES_BYTES * 2 + slen];
    for(uint32_t i = 0; i < AES_BYTES; ++i) {
        msg[i] = key[i];
    }
    for(uint32_t i = 0; i < AES_BYTES; ++i) {
        msg[i + AES_BYTES] = random_counter[i];
    }
    for(uint32_t i = 0; i < slen; ++i) {
        msg[i + AES_BYTES * 2] = sig[i];
    }

    //prepare the message = key, random_counter, sig
	connection->Send(msg, AES_BYTES * 2 + slen);
    connection->Close();
